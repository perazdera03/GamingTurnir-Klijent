...
// Ucitava turnire i timove za dropdown menije
const fetchData = async () => {
  try {
    const [turniriRes, timoviRes] = await Promise.all([
...
// Salje POST zahtev za dodavanje novog meca
const handleSubmit = async (event) => {
  ...
  try {
    ...
    router.push("/mecevi").then(() => toast.success("Meč uspešno dodat!"));
  } catch (error) {
    toast.error("Greška pri dodavanju meča.");
  }
};
