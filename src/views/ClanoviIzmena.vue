...
const id = route.params.id; // ID clana koji se menja
...
// Ucitava korisnike, timove i podatke clana koji se menja
const fetchData = async () => {
  try {
    const [korisnaciRes, timoviRes, clanRes] = await Promise.all([
      axios.get("/api/Korisnici"),
      axios.get("/api/Timovi"),
      axios.get(`/api/ClanoviTima/${id}`),
    ]);
    korisnici.value = korisnaciRes.data;
    timovi.value = timoviRes.data;
    const clan = clanRes.data;
    korisnikId.value = clan.korisnikId;
    timId.value = clan.timId;
    uloga.value = clan.uloga;
  } catch (error) {
    toast.error("Greška pri učitavanju podataka.");
  }
};

// Salje PUT zahtev za izmenu clana tima
const handleSubmit = async (event) => {
  event.preventDefault();
  try {
    await axios.put(`/api/ClanoviTima/${id}`, {
      korisnikId: parseInt(korisnikId.value),
      timId: parseInt(timId.value),
      uloga: uloga.value,
    });
    router.push("/clanovi").then(() => toast.success("Član uspešno izmenjen!"));
  } catch (error) {
    toast.error("Greška pri izmeni člana.");
  }
};
...
<template>
  <FormContainer title="Izmena člana tima">
